...
from src.users.schemas import BaseUserDetails, GuestUserCreate, GuestUserUpdate
...
class UserService:
    ...
    def list_selectable_users(
        ...
        *,
        ...
        users_stmt = select(User).order_by(User.id).offset((page - 1) * limit).limit(limit)
        if filters:
            users_stmt = users_stmt.where(*filters)
        users = self.db.execute(users_stmt).scalars().all()

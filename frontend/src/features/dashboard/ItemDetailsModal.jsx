...
export default function ItemDetailsModal({ isOpen, onClose, item, user, lang, onUpdateStatus }) {
    const [returnDate, setReturnDate] = useState('');

    // Domyślna data zwrotu (jutro)
    useEffect(() => {
        if (isOpen) {
    ...
    const t = {
        PL: {
            ...
            ownerDesc: 'Zarządzaj statusem i wypożyczeniami tego urządzenia.',
            reqPending: 'Nowy wniosek o wypożyczenie',
            reqDesc: `${item.borrower || 'Ktoś'} prosi o wypożyczenie tego sprzętu.`,
            btnAccept: 'Akceptuj (Zarezerwuj)',
            btnReject: 'Odrzuć wniosek',
            handover: 'Sprzęt zarezerwowany',
            handoverDesc: 'Oczekuje na fizyczne wydanie użytkownikowi.',
            btnGive: 'Potwierdź wydanie',
            returnPending: 'Sprzęt w terenie',
            returnDesc: `Wypożyczył(a): ${item.borrower}. Termin zwrotu: ${item.dueDate || 'Brak'}.`,
            btnReturn: 'Potwierdź zwrot',
            markDamaged: 'Zgłoś usterkę',
            qrCode: 'Identyfikator Systemowy',
            close: 'Zamknij',
            borrowPanel: 'Wypożycz Sprzęt',
            borrowDesc: 'Wskaż planowaną datę zwrotu, aby wysłać wniosek o wypożyczenie do właściciela urządzenia.',
            dateLabel: 'Data zwrotu',
            btnSubmitReq: 'Złóż wniosek o wypożyczenie',
            readOnlyDesc: `Opiekunem prawnym jest ${item.owner}. Nie posiadasz uprawnień do operacji na tym sprzęcie.`
        },
        EN: {
            ...
            ownerDesc: 'Manage the status and rentals of this device.',
            reqPending: 'New Rental Request',
            reqDesc: `${item.borrower || 'Someone'} requested to borrow this asset.`,
            btnAccept: 'Approve (Reserve)',
            btnReject: 'Reject Request',
            handover: 'Asset Reserved',
            handoverDesc: 'Waiting for physical handover to the user.',
            btnGive: 'Confirm Handover',
            returnPending: 'Asset Borrowed',
            returnDesc: `Borrowed by: ${item.borrower}. Due date: ${item.dueDate || 'N/A'}.`,
            btnReturn: 'Confirm Return',
            markDamaged: 'Report Damage',
            qrCode: 'System Identifier',
            close: 'Close',
            borrowPanel: 'Borrow Equipment',
            borrowDesc: 'Specify your planned return date to submit a rental request to the owner.',
            dateLabel: 'Return Date',
            btnSubmitReq: 'Submit Rental Request',
            readOnlyDesc: `The legal owner is ${item.owner}. You do not have permission to modify this asset.`
        }
    }[lang];
    ...
    const handleRequestRental = () => {
        ...
        // Zmienia status na "oczekuje akceptacji" i zapisuje imię zalogowanego usera oraz datę
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-900 flex justify-between items-start bg-slate-50/50 dark:bg-slate-900/20">
                    <div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t.detailsTitle}</div>
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${statusStyles}`}>
                ...
                <div className="p-6 overflow-y-auto flex-grow flex flex-col md:flex-row gap-6">
                    <div className="flex-1 space-y-6">
                        <div>
                            <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase border-b border-slate-100 dark:border-slate-800 pb-2 mb-3">{t.techSpec}</h4>
                            <ul className="space-y-3 text-sm">
                                <li className="flex justify-between"><span className="text-slate-500">{t.producer}</span> <span className="font-medium text-slate-800 dark:text-slate-200">{item.producer}</span></li>
                                <li className="flex justify-between"><span className="text-slate-500">{t.model}</span> <span className="font-medium text-slate-800 dark:text-slate-200">{item.model}</span></li>
                                <li className="flex justify-between"><span className="text-slate-500">{t.sn}</span> <span className="font-mono text-slate-800 dark:text-slate-200">{item.serialNumber}</span></li>
                                <li className="flex justify-between"><span className="text-slate-500">{t.category}</span> <span className="text-slate-800 dark:text-slate-200">{item.category}</span></li>
                            </ul>
                        </div>

                        <div>
                            <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase border-b border-slate-100 dark:border-slate-800 pb-2 mb-3">{t.location}</h4>
                        ...
                        <div className="pt-4 flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t.qrCode}</span>
                        </div>
                    </div>

                    {/* DYNAMICZNY PANEL PRACOWNICZY */}
                    {isOwner ? (
                        <div className="flex-1 bg-slate-50 dark:bg-slate-900/80 border border-emerald-100 dark:border-emerald-900/30 rounded-xl p-5 flex flex-col">
                            <div className="flex items-center space-x-2 mb-1">
                                ...
                                <h3 className="font-bold text-emerald-800 dark:text-emerald-400">{t.ownerPanel}</h3>
                            </div>
                            <p className="text-xs text-slate-500 mb-6">{t.ownerDesc}</p>
                            ...
                            <div className="space-y-4 flex-grow">
                                {item.status === 'oczekuje akceptacji' && (
                                    <div className="bg-white dark:bg-slate-950 border border-amber-200 dark:border-amber-900/50 rounded-lg p-4 shadow-sm">
                                        <h4 className="font-bold text-sm text-amber-700 dark:text-amber-500 mb-1">{t.reqPending}</h4>
                                        <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">{t.reqDesc}</p>
                                ...
                                {item.status === 'zarezerwowany' && (
                                    <div className="bg-white dark:bg-slate-950 border border-purple-200 dark:border-purple-900/50 rounded-lg p-4 shadow-sm">
                                        <h4 className="font-bold text-sm text-purple-700 dark:text-purple-500 mb-1">{t.handover}</h4>
                                        <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">{t.handoverDesc}</p>
                                        <button onClick={() => onUpdateStatus(item.id, 'wypożyczony')} className="w-full py-2 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded shadow-sm transition">{t.btnGive}</button>
                                    </div>
                                )}
                                {item.status === 'wypożyczony' && (
                                    <div className="bg-white dark:bg-slate-950 border border-blue-200 dark:border-blue-900/50 rounded-lg p-4 shadow-sm">
                                        <h4 className="font-bold text-sm text-blue-700 dark:text-blue-500 mb-1">{t.returnPending}</h4>
                                        <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">{t.returnDesc}</p>
                                        <button onClick={() => onUpdateStatus(item.id, 'dostępny', true)} className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded shadow-sm transition">{t.btnReturn}</button>
                                    </div>
                                )}
                        ...
                        <div className="flex-1 bg-white dark:bg-slate-950 border border-blue-200 dark:border-blue-900/50 rounded-xl p-5 flex flex-col shadow-sm">
                            <div className="flex items-center space-x-2 mb-1">
                                ...
                                <h3 className="font-bold text-blue-800 dark:text-blue-400">{t.borrowPanel}</h3>
                            </div>
                            <p className="text-xs text-slate-500 mb-6">{t.borrowDesc}</p>

                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 mb-1.5">{t.dateLabel}</label>
                                    <input
                                        type="date"
                                        value={returnDate}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/20">
                            ...
                            <p className="text-xs text-slate-500 mt-1">{t.readOnlyDesc}</p>
                        </div>
                    )}
                </div>
